...
export type ICreateInfiniteList<ITEM> = {
  ...
  loading: {
    start: (loadingAction: Loading, cursor?: string) => void;
    complete: (items: ITEM[], cursor?: string) => void;
    fail: () => void;
  };
};
...
const infiniteListStoreCreator = <ITEM>(
  ...
  const _set = (list: List<ITEM>) => {
    set({
      items: params?.processingBeforeSet ? params.processingBeforeSet(list.items) : list.items,
      cursor: list.cursor,
      loading: list.loading,
    });
  };

  const start = (loading: Loading, cursor?: string) => {
    const items = loading === Loading.initial ? [] : get().items;
    _set({loading, cursor, items});
  };

  const complete = (items: ITEM[], cursor?: string) => {
    const loading = Loading.idle;
    const loadingAction = get().loading;
    const result = isRefreshing(loadingAction) ? items : [...get().items, ...items];
    _set({loading, cursor, items: result});
  };
  const fail = () => {
    _set({loading: Loading.failed, cursor: undefined, items: initialState.items});
  };
...
export const createInfiniteListWithCursor = <ITEM, State, K extends keyof State>(
